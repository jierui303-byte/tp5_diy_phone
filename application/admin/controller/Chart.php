...
class Chart extends Base
{
    ...
    {
        $arrS = (new ChartCategory())->getAllListsByWhere(
            [
                'status' => 1
            ],
    ...
    public function show($id)
    {
        ...
        $picList = (new ChartPicture())->getAllListsByWhere(
            ['cate_id'=>$id],
    ...
    public function uploadChartPicture()
    {
        ...
        $newLeavePath = ROOT_PATH . 'uploads'.DS.'chart';//定义新的存储路径 /uploads/mask 蒙版目录下
        if($file){
...
//                echo $info->getFilename();
                $newPath = $this->request->domain().'/uploads'.DS.'chart/'.$info->getSaveName();
                //上传成功后，把图片最新路径和分类id分别存储在数据库中
                $res = (new ChartPicture())->insert(array(
    ...
    public function addChartPicture($id)
    {
        $cate_id = $id;//分类id
        $arrS = (new ChartCategory())->getAllListsByWhere(
            ['status' => 1],
        ...
        return $this->fetch('add-chart-picture');
    }
    ...
    public function addCategory()
    {
        if ($this->request->isAjax()){
            ...
            $res = (new ChartCategory())->insert($data);//新增
            if($res){
                $this->success('新增成功', 'admin/chart/index');//成功跳转
            }else{
    ...
    public function editCategory($id)
    {
        if ($this->request->isAjax()){
            ...
            $res = (new ChartCategory())
                ->where('id', $data['id'])
                ->update($data);//更新
            if($res){
                $this->success('修改成功', 'admin/chart/index');//成功跳转
            }else{
                $this->error('修改失败');//失败跳转
            }
        }else{
            //获取分类信息
            $res = (new ChartCategory())->getOneById($id, ['id,cate_name']);
            $token = $this->request->token('__token__', 'sha1');//生成token 自定义令牌生成规则，可以调用Request类的token方法
            $this->assign('token', $token);
